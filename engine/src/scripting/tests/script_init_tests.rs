...
fn test_script_properties_preserve_values_on_reinit() {
    ...
    props
        .values
        .insert("speed".to_string(), PropertyValue::Float(5.0)); // Changed from default 1.0
    props
        .values
        .insert("count".to_string(), PropertyValue::Integer(42)); // Changed from default 0
...
fn test_script_properties_add_new_properties_preserve_existing() {
    ...
    props
        .values
        .insert("speed".to_string(), PropertyValue::Float(10.0));
    world.inner_mut().insert_one(entity, props).unwrap();
...
fn test_script_change_preserves_matching_properties() {
    ...
    props
        .values
        .insert("shared_prop".to_string(), PropertyValue::Float(7.5));
    props
        .values
        .insert("script_a_only".to_string(), PropertyValue::Integer(100));
    world.inner_mut().insert_one(entity, props).unwrap();

    // Now change to script B which has some overlapping properties
    world
        .inner_mut()
        .insert(entity, (ScriptRef::new("script_b"),))
        .unwrap();
    ...
    // Verify results
    {
        ...
        assert_eq!(
            props.values.get("shared_prop"),
            Some(&PropertyValue::Float(7.5))
        ); // Preserved!
        assert_eq!(
            props.values.get("script_b_only"),
