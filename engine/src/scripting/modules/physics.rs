...
use crate::physics::{PhysicsCommand, PhysicsCommandQueue};
use glam::Vec3;
...
/// Create a physics module with command queue access
pub fn create_physics_module(command_queue: PhysicsCommandQueue) -> Module {
    ...
    module.set_native_fn(
        ...
        move |entity: i64, force: Dynamic| -> Result<(), Box<EvalAltResult>> {
            ...
            queue.write().unwrap().push(PhysicsCommand::ApplyForce {
    ...
    module.set_native_fn(
        ...
        move |entity: i64, impulse: Dynamic| -> Result<(), Box<EvalAltResult>> {
            ...
            queue.write().unwrap().push(PhysicsCommand::ApplyImpulse {
                entity: entity as u64,
                impulse: impulse_vec,
            });
    ...
    module.set_native_fn(
        ...
        move |entity: i64, torque: Dynamic| -> Result<(), Box<EvalAltResult>> {
            ...
            queue.write().unwrap().push(PhysicsCommand::ApplyTorque {
                entity: entity as u64,
                torque: torque_vec,
            });
    ...
    module.set_native_fn(
        ...
        move |entity: i64, linear: Dynamic, angular: Dynamic| -> Result<(), Box<EvalAltResult>> {
            ...
            queue.write().unwrap().push(PhysicsCommand::SetVelocity {
                entity: entity as u64,
                linear: linear_vec,
                angular: angular_vec,
            });
    ...
    module.set_native_fn(
        "raycast",
        move |origin: Dynamic, direction: Dynamic, max_distance: f32| -> Result<Dynamic, Box<EvalAltResult>> {
...
fn parse_vec3_from_dynamic(value: Dynamic) -> Result<Vec3, Box<EvalAltResult>> {
    ...
    if let Ok(vec3) = value.clone().try_cast::<Vec3>() {
        return Ok(vec3);
    }
...
mod tests {
    ...
    fn test_parse_vec3_from_array() {
        let array = vec![Dynamic::from(1.0f32), Dynamic::from(2.0f32), Dynamic::from(3.0f32)];
