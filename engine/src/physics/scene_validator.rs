        ...
        if let Some(collider_value) = entity_data.components.get("Collider") {
            if let Ok(collider) = serde_json::from_value::<Collider>(collider_value.clone()) {
                ...
                if let Some(rigidbody_value) = entity_data.components.get("Rigidbody") {
                    if let Ok(rigidbody) =
                        serde_json::from_value::<Rigidbody>(rigidbody_value.clone())
                    {
        ...
        if let Some(floor) = nearest_floor {
            ...
            if distance > 10.0 {
                result.errors.push(ValidationError {
                    ...
                    error_type: ErrorType::NoOverlap {
...
fn check_initial_overlaps(
    ...
    for body in dynamic_bodies {
        let body_aabb = compute_aabb(body.position, body.scaled_half_extents);

        for collider in static_colliders {
            let collider_aabb =
                compute_aabb(collider.position, collider.half_extents * collider.scale);
    ...
    for (i, body1) in dynamic_bodies.iter().enumerate() {
        let body1_aabb = compute_aabb(body1.position, body1.scaled_half_extents);

        for body2 in dynamic_bodies.iter().skip(i + 1) {
            let body2_aabb = compute_aabb(body2.position, body2.scaled_half_extents);
...
fn add_suggestions(result: &mut SceneValidationResult) {
    ...
    if result
        .errors
        .iter()
        .any(|e| matches!(e.error_type, ErrorType::NoOverlap { .. }))
    {
    ...
    if result.warnings.iter().any(|w| w.warning.contains("thick")) {
        result
            .suggestions
            .push("Use scale Y=1.0 for floors and adjust half_extents instead".to_string());
    }
    ...
    if !result.errors.is_empty() {
        result
            .suggestions
            .push("Test with SCENE=your_scene cargo run to verify physics behavior".to_string());
    }
}
...
mod tests {
    ...
    fn test_thin_floor_warning() {
        ...
        let has_thin_warning = result.warnings.iter().any(|w| w.warning.contains("thick"));
