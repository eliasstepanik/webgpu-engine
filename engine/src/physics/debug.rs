...
/// Convert Rapier's Unit<Quaternion<f64>> to glam's Quat
fn rapier_rotation_to_quat(rotation: &rapier3d_f64::prelude::Rotation<f64>) -> Quat {
...
pub fn draw_physics_debug(
    ...
    // Draw colliders
    for (_entity, (collider, rb)) in world.query::<(&Collider, &RigidBody)>().iter() {
                ...
                match &collider.shape {
                    ColliderShape::Cuboid(half_extents) => {
                        draw_box(
                            debug_lines,
                            relative_pos_f32,
                            rapier_rotation_to_quat(rot),
                            *half_extents,
                            color,
                        );
                    }
                    ColliderShape::Sphere(radius) => {
                        draw_sphere(debug_lines, relative_pos_f32, *radius, color);
                    }
...
fn draw_sphere(debug_lines: &mut Vec<DebugLineData>, position: Vec3, radius: f32, color: Vec4) {
...
fn draw_capsule(
    ...
    for i in 0..SEGMENTS / 2 {
        ...
        let next_angle =
            ((i + 1) % (SEGMENTS / 2)) as f32 * std::f32::consts::PI / (SEGMENTS / 2) as f32;
